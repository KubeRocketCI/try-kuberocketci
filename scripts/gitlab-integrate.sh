#!/usr/bin/env bash
# Post-KRCI GitLab integration. Requires `make krci` done first (codebase-operator, the
# edp-tekton tasks, the chart-rendered GitServer/EventListener). Pairs with
# scripts/gitlab-up.sh. Adds what the Helm chart cannot express:
#   1. mount the GitLab CA into codebase-operator (the chart has no hook for its
#      volumes; gitfusion gets the same CA declaratively via gitfusion.volumes in values),
#   2. patch the upstream gitlab-set-status task (host parse + self-signed TLS),
#   3. onboard the krci-gitops Codebase (the chart has no `codebases` values section),
#   4. wire the GitLab Package Registry into the maven/python/npm/pnpm builds (settings
#      ConfigMaps + task patches; the Tasks are chart-owned, Helm SSA resets them, so
#      they are re-applied here). See docs/registry-integration.md.
# The GitServer/EventListener/Ingress come from edp-tekton.gitServers.
set -euo pipefail
...
# ssh-known-hosts ConfigMap (chart: knownHosts.*; not disableable). GitLab generates
# host keys per install, so they are pinned here from the running GitLab. The CM is
# chart-owned (helm upgrade resets it; this step re-applies); the operator picks up CM
# edits without a restart. No-op in release mode (no such ConfigMap).
...
if $KUBECTL -n "$NS" get cm "$KH_CM" >/dev/null 2>&1; then
  ...
  # Both address forms: the GitServer connect check dials :32222 (bracket form); git
  # clone/push dials :22 (bare hostname). Same gitlab svc.
...
# webhooks only); without the CA, project/webhook creation fails with x509 "unknown
# authority". The CA is mounted into /etc/ssl/certs (Go scans that dir in addition to
# the system bundle; public CAs keep working). Additive volume/mount: `make krci` keeps
# it (helm 3-way merge).
...
# The upstream task parses the ssh://…:32222/… git URL to host "ssh" and verifies TLS
# against the self-signed cert; pipelines abort at report-pipeline-start.
...
# group). Two layers per language:
#   settings ConfigMap  — what the ecosystem's own config language can express (URLs,
#                         credentials). Named gitlab-*-settings, referenced from
#                         values/edp-install.yaml (tekton.configs.*ConfigMap); not
#                         chart-rendered, so a plain apply suffices.
#   task patch          — what that config language cannot express (per-build
#                         CI_PROJECT_ID, TLS trust). Chart-owned: Helm SSA resets these
#                         on every `make krci`, hence --force-conflicts here.
# Requires secret ci-nexus + cm gitlab-ca (both from gitlab-up). docs/registry-integration.md.
...
patch_task() {  # $1 = task name, $2 = one-line summary
...
# edp-npm/edp-pnpm are the build tasks: resolve-only from the group registry (CA
# needed); pnpm also needs an explicit userconfig to read the .npmrc (the stock task
# sets none).
...
# The codebase-operator restarts after the CA patch; its Codebase validating webhook
# (vcodebase.kb.io) can lag the Deployment's Available condition by a few seconds, and
# a bare apply then fails with "context deadline exceeded". A server-side dry-run
# invokes the webhook; polled until it succeeds, then apply.
